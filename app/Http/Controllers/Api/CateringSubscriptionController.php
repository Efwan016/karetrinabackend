...
class CateringSubscriptionController extends Controller
{
    public function store(StoreCateringSubscribeRequest $request)
    {
        ...
        if (!$cateringTier) {
            return response()->json(['message' => 'Tier Package not found'], 404);
        }
        ...
        $endedAt = $startedAt->copy()->addDays($cateringTier->duration - 1);
        ...
        $validatedData['booking_trx_id'] = CateringSubscription::generateBookingTrxId();
